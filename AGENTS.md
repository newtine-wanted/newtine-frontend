...
- 데스크톱 전용 화면이나 다단 레이아웃을 추가하지 않는다. 큰 화면에서도 모바일 콘텐츠를 중앙에 표시하고 배경색은 콘텐츠 영역과 동일하게 유지한다.
- 공통 레이아웃은 `src/app/layout.tsx`에서 관리한다. 실제 콘텐츠를 출력하는 `<main>`만 `w-full max-w-app`으로 제한하며, 최대 너비는 `src/app/globals.css`의 `--container-app: 480px` 한곳에서 관리한다.
- 높이는 `min-h-dvh`를 기준으로 한다. 콘텐츠가 길어지면 문서 전체가 세로 스크롤되도록 하고, 기본적으로 body 스크롤 잠금이나 중첩 스크롤을 추가하지 않는다.
- 노치와 홈 인디케이터는 공통 레이아웃의 `env(safe-area-inset-*)` 패딩과 `viewportFit: "cover"`로 대응한다. 페이지에서 안전 영역 패딩을 중복 적용하지 않는다.
- 헤더·하단 내비게이션이 필요하면 콘텐츠 너비 안에 배치한다. 문서 스크롤을 유지하는 `sticky`를 우선 고려하고, `fixed` 사용 시 화면 가림과 안전 영역을 별도로 확인한다.
...
- 기본 `/` 화면은 아직 비어 있다. 임의의 제품 메뉴, 다크 모드, PWA 설정은 요청이나 기능 요구사항이 있을 때 추가한다.

## 기술 스택과 코드

- Next.js App Router와 TypeScript를 사용한다. `strict` 설정을 유지하고 불필요한 `any`를 피한다.
- 스타일링은 Tailwind CSS 4 유틸리티를 우선 사용한다. 공통 전역 스타일과 Tailwind 테마 확장은 `src/app/globals.css`에서 관리하고, 별도의 스타일 라이브러리를 임의로 추가하지 않는다.
...
- `src/components/ui`에는 버튼·모달처럼 도메인과 무관한 UI만 둔다. 기존 공용 UI로 충족할 수 있으면 재사용하고, 제품 용어·데이터 조회·유스케이스가 들어가면 `domain` 또는 `features`에 둔다.
- 여러 기능에서 공유하는 제품 개념은 `domain`에, 한 사용자 흐름에서만 쓰는 상태·API·컴포넌트는 해당 `src/features/<feature>` 가까이에 둔다.
- Pen 문서는 Pencil MCP로만 확인하고 `.pen` 파일을 직접 읽지 않는다.
...
- `.pen`의 `_Marker`는 주석이므로 제품 UI로 구현하지 않는다. Status Bar와 Home Indicator는 제품 UI로 구현하지 않고 실제 기기의 OS UI와 safe-area를 사용한다.

## 확인과 문서

- 에이전트는 변경에 맞춰 `npm run format:check`, `npm run lint`, `npm run typecheck`까지만 실행한다. `npm run build`와 빌드를 포함하는 `npm run check`는 실행하지 않는다.
- 화면 검증은 사용자가 직접 수행한다. 에이전트는 브라우저 실행·조작, 스크린샷, 뷰포트별 렌더링 등 시각적 검증을 수행하지 않는다.
