...
- 색상의 단일 원본은 `src/design-system/tokens.css`다. `.pen`의 `00 · 디자인 시스템` (`DSBoard1`)에 있는 Ink·Olive·Moss·Surface·Muted 팔레트를 사용한다. 컴포넌트 스펙의 회색은 와이어프레임 값이므로 브랜드 역할에 매핑한다.
- 제품 코드에서는 `bg-primary`, `text-foreground`, `border-border` 같은 의미별 토큰만 사용한다. HEX/RGB/HSL 직접 입력, 임의 색상 클래스, Tailwind 기본 색상(`zinc`, `gray`, `red`, `blue` 등)은 사용하지 않는다. 색상 변경은 토큰 원본에서 수행한다.
- 토큰 내부의 파생 색상은 팔레트 기반 `color-mix`로 정의한다. 주제 칩의 임시 녹색·노랑·보라 와이어프레임 값은 그대로 복사하지 않는다. 빨강·파랑으로 정치적 분류나 오류 상태를 표현하지 않는다.
- 주제 이름·순서·색상 톤의 단일 원본은 `src/design-system/topics.ts`의 12종 분류다. 화면별로 분류를 재정의하지 않는다.
- 크기·간격·변형은 `.pen`의 선택된 컴포넌트 라이브러리와 스펙 시트를 따른다. 색상은 위 팔레트 원칙을 우선한다. 폰트는 로컬 패키지로 제공하는 Noto Sans KR을 사용한다.
