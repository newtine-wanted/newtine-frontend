...
- 이슈 상세는 `/issues/[issueId]`의 독립 페이지로 제공하며 피드와 관심 뉴스 목록에서 같은 경로를 공유합니다.
...
3. 액세스 토큰은 `session.ts`의 비공개 메모리에 저장하고, 인증 상태와 사용자 정보만 Zustand 스토어에 저장합니다.
...
| 값                    | 저장 위치                                | 프런트엔드 접근           | 용도                            |
| --------------------- | ---------------------------------------- | ------------------------- | ------------------------------- |
| 액세스 토큰           | `session.ts` 모듈의 비공개 메모리        | 세션 모듈 내부에서만 사용 | 보호 API의 `Authorization` 헤더 |
| 인증 상태·사용자 정보 | 메모리 전용 Zustand 스토어               | UI에서 구독 가능          | 인증 화면과 사용자 정보 표시    |
| 리프레시 토큰         | 백엔드가 설정한 `HttpOnly`·`Secure` 쿠키 | 직접 읽거나 저장하지 않음 | 액세스 토큰 재발급              |
| 이메일·비밀번호       | 저장하지 않음                            | 로그인 요청 중에만 사용   | 사용자 인증                     |

액세스 토큰은 Zustand, `localStorage`, `sessionStorage` 또는 프런트엔드에서 생성한 쿠키에 저장하지 않습니다. UI가 직접 읽을 수 없는 세션 모듈의 비공개 메모리에서만 관리합니다. 새로고침하면 메모리의 액세스 토큰은 사라지며, 리프레시 쿠키를 이용해 세션을 다시 복구합니다.
...
현재 `expiresIn`은 응답으로 받지만 별도로 저장하거나 사전 갱신 타이머에 사용하지 않습니다. 토큰 갱신은 앱 시작과 보호 API의 `401` 응답을 기준으로 수행합니다.
